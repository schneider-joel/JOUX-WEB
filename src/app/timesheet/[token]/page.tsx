  ...
  return (
    <>
      <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#aaa', padding: 8, background: '#fff' }}>
        DEBUG {new Date().toISOString()} · url={process.env.NEXT_PUBLIC_SUPABASE_URL} · key={(process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '').slice(0, 25)} · proyectos={proyectos?.length ?? 'null'} · rawCount={rawCount ?? 'null'} · error={proyectosError ? JSON.stringify(proyectosError) : 'none'} · rawErr={rawErr ? JSON.stringify(rawErr) : 'none'}
      </div>
